...
char* JWT::createGCPJWT(const char* projectId, const uint8_t* privateKeyFile, size_t privateKeySize, uint8_t expireInHours, unsigned char* jwtSeed, size_t myJwtSeedLen) {
    ...
    //mbedtls_ctr_drbg_seed(&ctr_drbg,mbedtls_entropy_func,&entropy,(const unsigned char*)pers,persLen);
...
int JWT::randomish( void *p_rng, unsigned char *output, size_t output_len ){
//int JWT::mbedtls_ctr_drbg_random_ish( void *p_rng, unsigned char *output, size_t output_len ){
  int randomish = Time.now();
  pinMode(A5, INPUT);
  for (uint8_t i = 0; i < output_len; i ++){
    output[i] = (unsigned char)(randomish * analogRead(A5));
  }
}
//}
